...
function renderTasks(tasks) {
    // get the list elemnt from the DOM
    // loop through the tasks array. transform (map) each task object into the appropriate HTML to represent a to-do.
    const todoList = document.getElementById('todoList');
    todoList.innerHTML = ''; //clears existing tasks
}
...
function newTask() {
    ...
    if (taskInput) {
        ...
        renderTasks(tasks);
        document.getElementById('todo').value = ''; //will clear input field
    }
}
...
function completeTask(taskElement) {
    ...
    tasks[taskIndex].completed = tasks[taskIndex]. completed ? flase : true;
...
function manageTasks(event) {
    ...
    // event.target will point to the actual icon clicked on. We need to get the parent li to work with however. HINT: Remember element.closest()? Look it up if you dont
